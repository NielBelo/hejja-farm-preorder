...
export default async function AdminSeasonsPage() {
  ...
  const [seasonsResult, pickupResult, ordersResult] = await Promise.all([supabase.from("season_parameters").select("*").order("year", { ascending: false }), supabase.from("pickup_days").select("id, season_parameter_id, pickup_date, planned_stock, available_stock, is_active").order("pickup_date"), supabase.from("orders").select("pickup_day_id, status")]);
  ...
  // különbsége (utóbbit a lemondott rendelések visszaadják).
